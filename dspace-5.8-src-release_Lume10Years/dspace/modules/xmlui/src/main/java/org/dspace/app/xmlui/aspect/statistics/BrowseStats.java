...
public class BrowseStats extends AbstractDSpaceTransformer 
{
	private final static Logger log = Logger.getLogger(BrowseStats.class);
		...
		IOException, AuthorizeException 
	{
		...
		{
			log.error("Error occurred while creating statistics in addBody BrowseStats.java", iae);
		}
	}
	...
	private void buildLinkToBrowse(Division currentDiv) throws WingException, UnsupportedEncodingException 
	{
		currentDiv.addPara().addXref(super.generateURL(contextPath + "/browse", getParams(false)), 
			getTitleMessage(browseInfo));
	}
	
	private void buildFilterOptions(Division currentDiv) throws WingException, 
		UnsupportedEncodingException, SQLException
	{
		...
		String linkUrl = contextPath + "browsestats";
		
		linkUrl = super.generateURL(linkUrl, getParams(true));
		
		para.addXref(linkUrl + "&stats_type=0", message("xmlui.Acessos.stats.agruparporano"));
		para.addXref(linkUrl + "&stats_type=1", message("xmlui.Acessos.stats.agruparpormes"));	
		...
		try 
		{
			...
			Division query = currentDiv.addInteractiveDivision("stats-filter", super.generateURL(contextPath + 
				"browsestats", getParams(false)) + "&stats_type=" + stats_type, Division.METHOD_POST, "browse stats");
	...
	{
		String urlShowAll = super.generateURL(contextPath + "browsestats", getParams(true)) + "&stats_type=" + stats_type;
		
		if (showAllCountries.equals("0")) 
		{
			currentDiv.addPara().addXref(urlShowAll.replace("showAll=0","showAll=1"), message("xmlui.Acessos.stats.vertodosospaises"));
		} 
		else 
		{
			currentDiv.addPara().addXref(urlShowAll.replace("showAll=1","showAll=0"), message("xmlui.Acessos.stats.naovertodosospaises"));
		}
	}
	...
	private Map<String,String> getParams(boolean withDate) throws UnsupportedEncodingException 
	{
		...
		params.put("value", URLEncoder.encode(value, "UTF-8"));
	...
	private ArrayList collectCountryData(String type) 
		throws SQLException, WingException 
	{
		...
		try 
		{
			...
			{
				/*ObjectCount[] downloadsPerCountry = SolrLogger.queryFacetField(query, 
					stats_type.equals("0") ? StringUtils.buildSolrDateFilterPerYear(dates) : StringUtils.buildSolrDateFilter(dates), 
				...
				ObjectCount[] downloadsPerCountry = SolrLogger.queryFacetField(query, StringUtils.buildSolrDateFilter(dates, stats_type), 
					"countryCode", showAllCountries.equals("0") ? 10 : Integer.MAX_VALUE, false, null);
				/*long totalUFRGS = SolrLogger.queryTotal(query, 
					stats_type.equals("0") ? StringUtils.buildSolrDateFilterPerYear(dates) : StringUtils.buildSolrDateFilter(dates)).getCount();*/
...
*/
	...
	{    
		...
		{
			log.error("Error occurred while creating statistics in (collectData) BrowseStats.java", e);
		}
		return result;
    }
	...
	{
		String title = "";
		Date startDate = new GregorianCalendar(Integer.parseInt(StringUtils.getSdate(0,dates)), 
				Integer.parseInt(StringUtils.pad(StringUtils.getSdate(1,dates), 2, 0)), 01).getTime();
		Date endDate = new GregorianCalendar(Integer.parseInt(StringUtils.getSdate(2,dates)), 
				Integer.parseInt(StringUtils.pad(StringUtils.getSdate(3,dates), 2, 0)), 31).getTime();
	...
	private TreeMap<String,String> getDownloadItems() 
	{
		...
		{
			log.error("Error occurred while creating statistics for browsestats in getDownloadItems.", e);
		} 
		return result;		
    }
	...
	private void collectHandles() 
	{
		try 
		{
			...
			//Codigo "herdado" do ConfigurableBrowse
			int sortBy = RequestUtils.getIntParameter(request, "sort_by");
			
			//Configura o indice que esta sendo buscado
			BrowseIndex bi = BrowseIndex.getBrowseIndex(type);
			if (bi == null) 
			{
				log.error("There is no browse index of the type: " + type);
			}
			...
			else if (bi.isItemIndex() && !bi.isInternalIndex()) 
			{
				...
				{
					log.error("Unable to get sort options", se);
				}
			}
			...
			//Define o valor que sera filtrado (nesse caso, nome do autor ou assunto
			scope.setFilterValue(value);
			//Define o nivel, 1 eh para autor ou assunto
			scope.setBrowseLevel(1);
			//Define quantidade de itens no resultado
			...
			//Nao sei exatamente pra que é isso
			browseInfo.setEtAl(-1);
			
			//Se for um "percorrer" para itens (sempre será, mas nao custa testar)
			//Adiciona os handles dos itens à uma lista
			if (isItemBrowse(browseInfo)) 
			{
				...
				for (BrowseItem item : (java.util.List<BrowseItem>) browseInfo.getResults()) 
				{
					...
					idHandleList.put(String.valueOf(item.getID()), item.getHandle());
				}
			}
		} 
		catch (Exception ex) 
	...
	private Message getTitleMessage(BrowseInfo info) 
	{
        ...
        if (titleMessage == null) 
		{
            ...
            if (bix.isMetadataIndex()) 
			{
				if (scopeName.equals("")) 
				{
					titleMessage = message("xmlui.Acessos.stats.title." + bix.getName())
                        .parameterize(value);
				} 
				else 
				{
					titleMessage = message("xmlui.Acessos.stats.title_scope." + bix.getName())
                        .parameterize(scopeName,value);
				}
            } 
			else if (info.getSortOption() != null) 
			{
                titleMessage = message("xmlui.Acessos.stats.title.item." + info.getSortOption().getName())
                        .parameterize(scopeName, value);
            } 
			else 
			{
                titleMessage = message("xmlui.Acessos.stats.title.item." + bix.getSortOption().getName())
                        .parameterize(scopeName, value);
            }
        }
